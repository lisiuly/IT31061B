...
C_RFCh1Sync57Sec		equ	57
C_RFCh2Sync67Sec		equ	67
C_RFCh3Sync79Sec		equ	79
C_RFCh1Sync57Tick		equ	114		; 57 秒，0.5 秒粒度。
C_RFCh2Sync67Tick		equ	134		; 67 秒，0.5 秒粒度。
C_RFCh3Sync79Tick		equ	158		; 79 秒，0.5 秒粒度。
...
; F_RF_ClearPendingFrame / F_RF_SavePendingFrame / F_RF_IsCurrentFrameConfirmed: 维护双帧确认缓存。
...
F_RF_HandleSelectedChannelFail:
		...
		STA		R_RF1LostCnt,X
		PLA
		JSR		F_RF_SetSelectedChannelLost
		RTS
...
; 输出: 按当前请求通道选择 57/67/79 秒常规同步周期。
; 调试: 看请求通道变化后，哪一路 SyncTm 被重新装载。
F_RF_ReloadRequestChannelSync:		; 按当前请求通道重装常规同步周期
		LDA		R_RFRequestChannel
		CMP		#D_RFCh2
		BEQ		RF_ReloadRequestChannel2Sync
		CMP		#D_RFCh3
		BEQ		RF_ReloadRequestChannel3Sync
		JMP		F_RF_ReloadChannel1Sync

RF_ReloadRequestChannel2Sync:
		JMP		F_RF_ReloadChannel2Sync

RF_ReloadRequestChannel3Sync:
		JMP		F_RF_ReloadChannel3Sync

; 输入: R_RFRequestChannel。
; 输出: 首次配对成功后，把该通道同步点装到“完整周期 - 2 秒提前量”。
; 调试: 长接收首次配对成功后，看对应 SyncTm 是否被写成周期减 4 tick。
F_RF_LoadRequestChannelFirstSync:	; 首次配对成功后装载“周期-2秒提前量”的下次同步点
		LDA		R_RFRequestChannel
		CMP		#D_RFCh2
		BEQ		RF_LoadRequestChannel2FirstSync
		CMP		#D_RFCh3
		BEQ		RF_LoadRequestChannel3FirstSync
		LDA		#C_RFCh1Sync57Tick-C_RFSyncAdvance2SecTick
		STA		R_RF1SyncTm
		RTS

RF_LoadRequestChannel2FirstSync:
		LDA		#C_RFCh2Sync67Tick-C_RFSyncAdvance2SecTick
		STA		R_RF2SyncTm
		RTS

RF_LoadRequestChannel3FirstSync:
		LDA		#C_RFCh3Sync79Tick-C_RFSyncAdvance2SecTick
		STA		R_RF3SyncTm
		RTS

; 这三个函数只负责把常规同步周期常量写回各自的 SyncTm。
; 调试: 看 R_RF1SyncTm / R_RF2SyncTm / R_RF3SyncTm 是否分别变成 114 / 134 / 158。
F_RF_ReloadChannel1Sync:		; 重装 CH1 的 57 秒常规同步周期
		LDA		#C_RFCh1Sync57Tick
		STA		R_RF1SyncTm
		RTS

F_RF_ReloadChannel2Sync:		; 重装 CH2 的 67 秒常规同步周期
		LDA		#C_RFCh2Sync67Tick
		STA		R_RF2SyncTm
		RTS

F_RF_ReloadChannel3Sync:		; 重装 CH3 的 79 秒常规同步周期
		LDA		#C_RFCh3Sync79Tick
		STA		R_RF3SyncTm
		RTS

;		旧的按请求通道分拆 Valid/Lost 入口已合并到
;		F_RF_SetSelectedChannelValid / F_RF_SetSelectedChannelLost，
;		不再保留 6 份完全重复的代码块。

; 这三个函数只是给通用分钟状态机装不同的 X/Y 偏移。
; 调试: 看 X/Y 是否落在 0/0、0Ah/01h、14h/02h。
...
; 输入: 当前帧 R_RFPacket0~4。
; 输出: 把当前帧拷到 pending 缓存，供下一帧做完全一致比较。
; 调试: 盯 R_RFPending0~4。
F_RF_SavePendingFrame:			; 保存当前帧，供下一帧做完全一致比对
		LDA		R_RFPacket0
		STA		R_RFPending0
		LDA		R_RFPacket1
		STA		R_RFPending1
...
; 输入: 当前帧和 pending 帧。
; 输出: SEC=两帧完全一致，CLC=不一致或这是第一帧。
; 调试: 重点对比 R_RFPacket0~4 和 R_RFPending0~4。
F_RF_IsCurrentFrameConfirmed:		; 判断当前帧是否和上一帧完全一致
		; 发射端会连续重复发同一帧，接收端必须连续两帧完全一致才允许后续落库。
		LDA		R_RFPendingValid
		; 还没有 pending 时，当前帧只保存，不算确认成功。
		BNE		RF_CheckPendingFrame
...
; 输入: R_RFRequestChannel 和当前帧里的 Packet0(ID)。
; 输出: SEC=允许写入该通道，CLC=拒绝；已配对通道只接受相同 ID。
; 调试: 看 R_RFRequestChannel、对应通道 Flags 的 NeedPair 位、对应通道 IdL。
F_RF_IsRequestChannelPacketAccepted:	; 判断当前包是否允许写入请求通道
...
F_RF_ParsePacket:			; 满 36bit 后做双帧确认，再按通道码和已绑定 ID 决定是否落库
		LDA		R_RFBitCnt
		CMP		#C_RFPacketBits36
		BCC		RF_ParsePacket_End
